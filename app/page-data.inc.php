...
Class PageData {
  ...
  static function current_language($page){
    $cl = explode('/', $page->url_path);
    $cl_dir = './content/'.$cl[0].'/';
    $dir_cl= null;
    if(is_dir($cl_dir)){
      $dir_cl = $cl[0];
  ...
  static function get_languages($page){
    $lang_dir = explode('/', $page->url_path);
    $dir_languages = './content/'.$lang_dir[0].'/';
    if(!is_dir($dir_languages) || !file_exists($dir_languages.'_languages.txt')){
      $dir_languages = './content/';
    }

    $lang = (file_exists($dir_languages.'_languages.txt')) ? file_get_contents($dir_languages.'_languages.txt') : '';
    $lang = explode("-", $lang);
    array_pop($lang);
    $languages = array();
  ...
  static function create_vars($page) {
    ...
    # @current_language
    $cl = self::current_language($page);
    $page->current_language = $cl;
    # @link_language
    $page->link_language = str_replace($cl, '', $page->url_path);
  ...
  static function create_collections($page) {
    ...
    # $languages
    $page->languages = self::get_languages($page);
  }
  ...
  static function create_textfile_vars($page) {
    ...
    $shared_dir = explode('/', $page->url_path);
    $dir_shared = './content/'.$shared_dir[0].'/';
    if(!is_dir($dir_shared)){
      $dir_shared = './content/';
    }
    $shared = (file_exists($dir_shared.'_shared.txt')) ? file_get_contents($dir_shared.'_shared.txt') : '';
